...
#include <hal/utils/net/ap_core.h>
...
// Wi-Fi stack bring-up and the AP SSID helper now live in net::ap_core so the
// STA sync path shares the exact same one-time initialiser (see ap_core.h).
bool ensure_wifi_stack_ready()
{
    return net::ap_core::ensureWifiStackReady();
}

std::string make_ap_ssid()
{
    return net::ap_core::makeApSsid();
}
