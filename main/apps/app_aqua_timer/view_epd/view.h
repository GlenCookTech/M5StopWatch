...
/*
 * E-ink counterpart of view/view.h for the M5Paper build: same classes, same
 * public API, so app_aqua_timer.cpp compiles against either. Layout is 540x960
 * portrait, black ink on white paper, no animations (marquees and spring
 * physics have no place on a panel that refreshes in ~120 ms), and cues are
 * rendered visually: interval type flips the page polarity and interval
 * changes trigger a full quality refresh whose inherent black/white flash is
 * the "new interval" signal.
 */
namespace view {
 ...
 *
 * Polarity is the long-distance channel (Work = black page, Rest = white
 * page), the full-refresh flash is the transition channel, text is the
 * close-range channel.
 *
 */
class RunningPage {
...
private:
    ...
    /* Last rendered values; only changes are pushed so the panel sees ~1
       small partial update per second. */
    ...
    size_t _shown_index                      = SIZE_MAX;
    bool _shown_inverted                     = false;
    bool _shown_chip                         = false;
    int32_t _shown_bar_value                 = -1;
    ...
    void applyPolarity(bool inverted, bool powerFrame);
    void setCountdownChip(bool show);
};
...
/**
 * @brief App home: start a class, sync, Wi-Fi setup.
 *
 * The cue-mode toggle of the AMOLED build is gone: this board has no speaker
 * and no vibration motor, cues are always visual.
 *
 */
class AquaMenuPage {
public:
...
private:
    ...
    void addButton(int y, const std::string& text, std::function<void()>* action, bool primary);
};
